...
public class FRM_AsignacionActivos extends javax.swing.JDialog {
    ...
    public FRM_AsignacionActivos(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public FRM_AsignacionActivos() {
        
    }
    
    //Bloquea acciones de copiar y pegar del teclado
    public void bloquearTeclas(){
        InputMap map1 = txt_serie_activo.getInputMap(txt_serie_activo.WHEN_FOCUSED);
        map1.put(KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.CTRL_MASK), "null");
        InputMap map2 = txt_codigoInternoInsticucional_activo.getInputMap(txt_codigoInternoInsticucional_activo.WHEN_FOCUSED);
    ...
    public void verificarBusqueda(){
        if (!rbtn_serie_activo.isSelected() && !rbtn_codigoinstitucional_activo.isSelected() && !rbtn_tipo.isSelected()) {
                mensajes.msg_advertencia(this,"Debe elegir un criterio de búsqueda.");
            } else{
                if ((rbtn_serie_activo.isSelected() && txt_serie_activo.getText().equals(""))||(rbtn_codigoinstitucional_activo.isSelected() && txt_codigoInternoInsticucional_activo.getText().equals(""))) {
                        mensajes.msg_advertencia(this,"Debe ingresar un dato para la búsqueda.");
                    } else{
                            buscar();
                        }
                }    
   ...
   public void cargarMarcas(){
            ...
            if (rbtn_marca_activo.isSelected()) {
                ...
                Integer numTipo = combo_tipo_activo.getSelectedIndex() + 1;
                    if (conexion.crearConexion()) {
                        ...
                        String sql="select  id_activo, idtipo_activo, marca_activo, precesador_acrtivo, memoria_activo, discoduro_activo, modelo_activo, serie_activo, costo_activo, fechacompra_activo, codigointernoinstitucional_activo from tmovactcon where marca_activo = '"+valMarca+"' and idtipo_activo = '"+numTipo+"' and iddocasignacion_activo = 2";
                        try{
                            ...
                            {
                                mensajes.msg_advertencia(this, "No existen registro para este criterio de búsqueda.");
                            }
                ...
                if (rbtn_procesador_activo.isSelected()) {
                    ...
                    if (conexion.crearConexion()) {    
                        ...
                        String sql="select  id_activo, idtipo_activo, marca_activo, precesador_acrtivo, memoria_activo, discoduro_activo, modelo_activo, serie_activo, costo_activo, fechacompra_activo, codigointernoinstitucional_activo from tmovactcon where precesador_acrtivo = '"+valProcesador+"' and iddocasignacion_activo = 2";
                        try{
                            ResultSet rs = conexion.ejecutarSQLSelect(sql);
                            if(rs.next())
                            {
                    ...
                    if (rbtn_memoria_activo.isSelected()) {
                        ...
                        if (conexion.crearConexion()) {
                            ...
                            String sql="select  id_activo, idtipo_activo, marca_activo, precesador_acrtivo, memoria_activo, discoduro_activo, modelo_activo, serie_activo, costo_activo, fechacompra_activo, codigointernoinstitucional_activo from tmovactcon where memoria_activo = '"+valMemoria+"' and iddocasignacion_activo = 2";
                            try{
                                ResultSet rs = conexion.ejecutarSQLSelect(sql);
                        ...
                        if (rbtn_discoduro_activo.isSelected()) {
                            ...
                            if (conexion.crearConexion()) {
                                ...
                                String sql="select id_activo, idtipo_activo, marca_activo, precesador_acrtivo, memoria_activo, discoduro_activo, modelo_activo, serie_activo, costo_activo, fechacompra_activo, codigointernoinstitucional_activo from tmovactcon where discoduro_activo = '"+valDiscoDuro+"' and iddocasignacion_activo = 2";
                                try{
                                    ResultSet rs = conexion.ejecutarSQLSelect(sql);
            ...
            if (rbtn_serie_activo.isSelected()) {
                String valSerie = txt_serie_activo.getText();
                    if (conexion.crearConexion()) {
                        ...
                        String sql="select  id_activo, idtipo_activo, marca_activo, precesador_acrtivo, memoria_activo, discoduro_activo, modelo_activo, serie_activo, costo_activo, fechacompra_activo, codigointernoinstitucional_activo from tmovactcon where serie_activo = '"+valSerie+"' and iddocasignacion_activo = 2";
                        try{
                            ResultSet rs = conexion.ejecutarSQLSelect(sql);                   
                                if(rs.next())
                                {
    ...
    private void btn_regresarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_regresarActionPerformed
        if(mensajes.msg_Pregunta(this, "¿Desea cerrar asignación de activos?")==0){
            dispose();
        }
    ...
    private void btn_asignarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_asignarActionPerformed
        int[] row = tabla_activoSinResponsable.getSelectedRows(); // toma el numero de columnas que estan sombreadas o seleccionadas
        int numero = row.length; // toma el numero de columnas seleccionadas
        if(numero>0){
            int id = 0;
            String s = "";
            for (int i = 0; i < (numero); i++) {// se realiza un for para el numeor de columnas que se tomaron
                id = Integer.parseInt(tabla_activoSinResponsable.getValueAt(row[i], 0).toString());// se toma el id de la columna seleccionada por medio de su ubicacion dentro de la tabla 
                FRM_RegistroDocumentoAsignacion.bandera = 1;
                FRM_RegistroDocumentoAsignacion.id_activoasignar.add(id);// se agreda cada una de las id del activo que se toman de cada columna seleccionda 
            }
            dispose();
            ...
            mensajes.msg_advertencia(this, "Debe seleccionar al menos un activo para poder asignar.");
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(FRM_AsignacionActivos.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                FRM_AsignacionActivos dialog = new FRM_AsignacionActivos(new javax.swing.JFrame(), true);
