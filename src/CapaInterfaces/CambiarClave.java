...
public class CambiarClave extends javax.swing.JDialog {
    ...
    public CambiarClave(java.awt.Frame parent, boolean modal, String var_nombreUsuario) {
        super(parent, modal);
        initComponents();
        ...
        this.usuarioLogueado=var_nombreUsuario;
        
        //bloquear combinacion copia y pega
        //Para la clave nueva
         InputMap map3 = txt_nuevaclave_usuario.getInputMap(txt_nuevaclave_usuario.WHEN_FOCUSED);
        map3.put(KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.CTRL_MASK), "null");
        //Para la confirmacion
         InputMap map4 = txt_rnuevaclave_usuario.getInputMap(txt_rnuevaclave_usuario.WHEN_FOCUSED);
    ...
    private void btn_actualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_actualizarActionPerformed
        ...
        //se extraen los datos de los textfield
        String claveActual = txt_clave_usuario.getText();
        String claveNueva = txt_nuevaclave_usuario.getText().trim();
        String claveConfirmacion = txt_rnuevaclave_usuario.getText();
      

        if (!"".equals(claveActual) && !"".equals(claveActual) && !"".equals(claveConfirmacion)) {

            if (usuario.validar_ingreso(usuarioLogueado, claveActual)!=0) {
                if (validar.ComplejidadClave(this, claveNueva)) {
                    if (validar.IgualdadCampos(this, claveNueva, claveConfirmacion)) {
                   
                        if (conexion.ejecutarSQL("Update TMAEUSUCON set clave_usuario = '"+claveNueva+"'  where nombre_usuario = '"+usuarioLogueado+"'")) {
                            limpiarCamposClaves();
                            mensaje.msg_confirmacion(this, "Clave Actualizada correctamente");
                        } else {
                            mensaje.msg_advertencia(this, "Actualizacion de datos fallida");
                        }
                    }
                }
            }

       }else{mensaje.msg_advertencia(this, "los campos no pueden estar vacios");}
    }//GEN-LAST:event_btn_actualizarActionPerformed

    
    private void limpiarCamposClaves(){
    txt_clave_usuario.setText(null);
    txt_nuevaclave_usuario.setText(null);
    txt_rnuevaclave_usuario.setText(null);
    }
